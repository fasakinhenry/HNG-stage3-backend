...
export async function listUsers(req: Request, res: Response): Promise<void> {
  try {
    ...
    const data = users.map((u: any) => ({ ...u, id: u._id, _id: undefined }));
...
export async function updateUserRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.params.id, { role }, { new: true }).lean() as any;
    if (!user) { sendError(res, 'User not found', 404); return; }
    res.json({ status: 'success', data: { ...user, id: user._id, _id: undefined } });
  } catch {
...
export async function updateUserStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.params.id, { is_active }, { new: true }).lean() as any;
    if (!user) { sendError(res, 'User not found', 404); return; }
    res.json({ status: 'success', data: { ...user, id: user._id, _id: undefined } });
  } catch {
