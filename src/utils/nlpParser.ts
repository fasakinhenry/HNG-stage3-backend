...
// Keyword maps
const GENDER_KEYWORDS: Record<string, 'male' | 'female'> = {
  male: 'male',
  males: 'male',
  men: 'male',
  man: 'male',
 ...
 */
function parseAgeRanges(q: string): { min_age?: number; max_age?: number } {
 ...
 */
function parseCountry(q: string): string | undefined {
  ...
  if (isoMatch) {
    ...
    if (COUNTRY_MAP[iso.toLowerCase()]) {
      return iso;
    }
  }

  // Try country name — search by longest match to avoid partial hits
  const sortedEntries = Object.entries(COUNTRY_MAP).sort(
...
export function parseNaturalLanguageQuery(q: string): ParsedQuery | null {
  ...
  if (ageRanges.min_age !== undefined) result.min_age = ageRanges.min_age;
  if (ageRanges.max_age !== undefined) result.max_age = ageRanges.max_age;

  // Country
  const country = parseCountry(q); // use original case for ISO detection
  if (country) result.country_id = country;
